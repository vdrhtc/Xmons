...
\caption{Equivalent circuit for coupled system of a tunable transmon qubit and a resonator. Colors show nodes (or branches) containing system's degrees of freedom according to Devoret's theory\cite{Devoret1995}.}
...
=  \hbar\omega_r\ \hat a^\dag \hat a \otimes \mathbbm{\hat 1}_q \\
+ E_C\ \mathbbm{\hat 1}_r \otimes \hat n^2 - \frac{E_J(\Phi_{ext})}{2}\ \mathbbm{\hat 1}_r\otimes \sum_{n=-\infty}^{+\infty} \ket{n+1}\bra{n} + \ket{n}\bra{n+1}  \\
- 2e \frac{C_g}{C_*} \sqrt{\frac{\hbar \omega_r }{2(C_q+C_g)}}\ i(\hat a^\dag - \hat a) \otimes \hat n, 
\end{gathered}\label{eq:hamiltonian}
\end{equation}
where 
$$C_*^2 = C_q C_g + C_q C_\kappa + C_g C_\kappa + C_q C_r + C_g C_r, $$
$$E_J(\Phi_{ext}) = E_{J,\Sigma} \cos(\Phi/\Phi_0),\ \Phi_{ext} = I_\Phi M.$$
Presuming the coupling is not very strong ($C_g \ll C_q, C_r$) it is possible also to include simple driving terms for both subsystems:
\begin{gather*}
\mathcal{\hat H}_r^d = \frac{C_\kappa V_\kappa(t)}{C_r + C_\kappa} \hat Q_r \propto f_r(t)\ i(\hat a^\dag - \hat a) \otimes \mathbbm{\hat 1}_q \\
\mathcal{\hat H}_q^d = \frac{C_e V_e(t)}{C_q + C_e} \hat Q_q \propto f_q(t)\  \mathbbm{\hat 1}_r \otimes \hat n,
\end{gather*}
where $f_{q, r} (t)$ are the effective values of the drive magnitude.

\section{Energy spectrum of the compound system}

Using QuTiP\cite{Johansson2011} it is straightforward to solve the truncated matrix eigenproblem with the Hamiltonian \eqref{eq:hamiltonian}. The results for the XMon design values
$$
C_\kappa = 5 fF,\ C_g = 2 fF,\ C_q = 90 fF,\ C_r = 500 fF,\ L_r = 20 nH
$$
...
So what happens in reality? Do we observe entanglement? The spin-echo experiment which is capable of reducing the classical noise effects (\autoref{fig:se}) tells us that reversible dephasing exists. Obviously, entanglement also should exist as a fundamental concept. It's possible that the slow components of the bath's noise do not lead to entanglement, they only shift the energy by a constant random value on the time scale of a single evolution experiment. However fast components lead to irreversible dephasing and entanglement.

However it is unclear why quantum theory does not generalize the classical. Maybe Bloch-Redfield ME will help.
