...
So what happens in reality? Do we observe entanglement? The spin-echo experiment which is capable of reducing the classical noise effects (\autoref{fig:se}) tells us that we have both. The slow components of the bath's noise do not lead to entanglement, they only shift the energy by a constant random value on the time scale of a single evolution experiment. However fast components lead to irreversible dephasing and entanglement.

However it is unclear why quantum theory does not generalize the classical.
...
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=0.9\textwidth]{cse_xyz_rf}
\caption{Same in $xyz$. Dephasing is linear in time and reversible for this case.}
\end{subfigure}
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=0.9\textwidth]{qse_xyz_rf}
